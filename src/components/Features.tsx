...
const advantages = [
  ...
  },
  {
    ...
    desc: "Клиенты, сделки, процессы и доступ к рабочей среде собраны в одном понятном контуре.",
  },
  {
    icon: Database,
...
export const Features = () => {
  ...
  return (
    <section
      ...
      ref={container}
      className="py-28 md:py-36 px-6 sm:px-12 md:px-24 bg-white overflow-hidden"
    >
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {advantages.map((adv, i) => (
            <div
              key={i}
              className="advantage-card group relative p-10 bg-[#FBFBFA] hover:bg-[#F6F5F4] rounded-[2rem] border border-primary/5 transition-all duration-700 flex flex-col justify-between items-start min-h-[320px]"
            >
              <div className="flex flex-col gap-6">
                <div className="w-12 h-12 rounded-2xl bg-white border border-primary/5 flex items-center justify-center group-hover:bg-accent group-hover:scale-110 transition-all duration-500">
                  <adv.icon className="w-5 h-5 text-accent group-hover:text-white transition-colors" />
                </div>
                <h3 className="text-lg font-sans font-black text-primary leading-tight uppercase tracking-tight group-hover:text-accent transition-colors">
                  {adv.title}
                </h3>
              </div>
